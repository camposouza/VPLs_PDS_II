...
class FiltroNumAbundante: public Filtro<int> {
  bool dadoValido(int& num) const override {
    ...
    for (int i = 1; i <= num/2; i++) {
      if (num/i == 0) { somaAbundante += i; }
    }
    return somaAbundante > num ? true : false;
  }
};
...
class FiltroNumTriangular: public Filtro<int> {
  bool dadoValido(int& num) const override {
    int somaTriangular = 0, somador = 1;
    while (true) {
      somaTriangular += somador;
      somador++;
      if (somaTriangular == num) { return true; }
      else if (somaTriangular > num) { return false; }
    }
    return false;
  }
};
